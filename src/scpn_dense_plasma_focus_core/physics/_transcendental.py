...
# SCPN Dense Plasma Focus Core — vendored deterministic logarithm, exponential and power
...
Provenance: byte-identical in every operation to
``scpn_reactor_kernels.numerics.transcendental`` of SCPN-REACTOR-KERNELS at
its commit ``799d44d3`` (the canonical copy); only the provenance header
and the error import differ. This copy is temporary: it is retired in
favour of the pinned library once the library can be pinned by version and
digest, exactly as the geometry copies of the Z-pinch family.
"""
