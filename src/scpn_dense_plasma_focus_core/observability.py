...
# SCPN Dense Plasma Focus Core — diagnostic and clock semantics
...
identities for the dense-plasma-focus configuration this repository owns, aligned
...
from scpn_dense_plasma_focus_core.configuration import ConsistencyFinding
...
INSTABILITY_BAND_HZ: Final = (1.0e06, 1.0e09)
DRIVE_TIMING_CEILING_S: Final = 1.0e-08

SHOT_DURATION_CEILING_S: Final = 1.0e-04
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-DENSE-PLASMA-FOCUS-CORE"
                )
    ...
    def __post_init__(self) -> None:
        ...
        if candidate is None:
            raise DiagnosticPlanError(
                ...
                "to SCPN-DENSE-PLASMA-FOCUS-CORE"
            )
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-DENSE-PLASMA-FOCUS-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented plasma-focus diagnostic practice; empty
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "Hz is outside the m=0 necking and "
                            "pinch-phase scale 1-1000 MHz "
                            "(Lee and Serban 1996)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            " is coarser than the ns-scale "
                            "rundown-to-pinch timing "
                            "(Lee and Serban 1996)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the plasma-focus discharge scale of up "
                            "to ~100 us (Lee and Serban 1996)"
                        ),
                    )
                )
