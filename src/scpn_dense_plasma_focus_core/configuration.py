...
# SCPN Dense Plasma Focus Core — device configuration container

"""Device configuration container bound to the SPO reactor registry.

A :class:`DeviceConfiguration` composes a validated electrode set and
bank/fill declaration under the single registry identifier this
repository owns. The coaxial ordering is hard; for deuterium fills the
Lee-Serban drive parameter outside the documented window of optimised
machines is flagged (Lee & Serban, IEEE TPS 24 (1996) 1101).
...
OWNED_CONFIGURATIONS: Final = ("dense_plasma_focus",)
DEUTERIUM_DRIVE_WINDOW: Final = (66.0, 108.0)
...
class DeviceConfiguration:
    """Validated dense-plasma-focus device configuration.
    ...
    identifier
        ...
        ``dense_plasma_focus``.
    electrodes
        Validated coaxial electrode set.
    bank
        Validated bank/fill declaration.
    registry
    ...
    def __post_init__(self) -> None:
        ...
        if self.identifier not in OWNED_CONFIGURATIONS:
            raise DeviceConfigurationError(
                ...
                f"SCPN-DENSE-PLASMA-FOCUS-CORE; owned: {OWNED_CONFIGURATIONS!r}"
            )

    def drive_parameter(self) -> float:
        """Lee-Serban drive parameter of the configuration.
        ...
        """
        return self.bank.drive_parameter(self.electrodes)
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            ...
            a deuterium drive parameter sits in the documented window,
            and always empty for non-deuterium fills. Findings are
            advisory instruments, not machine claims.
        """
        findings: list[ConsistencyFinding] = []
        if self.bank.deuterium_fill:
            drive = self.drive_parameter()
            low, high = DEUTERIUM_DRIVE_WINDOW
            if not low <= drive <= high:
                findings.append(
                    ConsistencyFinding(
                        field="bank.peak_current_ma",
                        message=(
                            f"drive parameter {drive:.1f} kA/cm/sqrt(Torr) "
                            f"is outside the documented deuterium window "
                            f"[{low:.0f}, {high:.0f}]"
                        ),
                    )
                )
    ...
    def to_record(self) -> dict[str, Any]:
        ...
        return {
            ...
            "electrodes": {
                "anode_radius_m": self.electrodes.anode_radius_m,
                "cathode_radius_m": self.electrodes.cathode_radius_m,
                "anode_length_m": self.electrodes.anode_length_m,
            },
            "bank": {
                "bank_energy_kj": self.bank.bank_energy_kj,
                "peak_current_ma": self.bank.peak_current_ma,
                "fill_pressure_torr": self.bank.fill_pressure_torr,
                "deuterium_fill": self.bank.deuterium_fill,
            },
            "registry": {
...
def configuration_from_record(record: Any) -> DeviceConfiguration:
    ...
    known = {"identifier", "electrodes", "bank", "registry"}
    unknown = sorted(set(record) - known)
    if unknown:
        raise DeviceConfigurationError(f"record: unknown fields {unknown!r}")
    electrodes = _require_mapping(record, "electrodes")
    bank = _require_mapping(record, "bank")
    registry = _require_mapping(record, "registry")
    return DeviceConfiguration(
        identifier=_string(record, "identifier"),
        electrodes=ElectrodeSet(
            anode_radius_m=_number(electrodes, "anode_radius_m"),
            cathode_radius_m=_number(electrodes, "cathode_radius_m"),
            anode_length_m=_number(electrodes, "anode_length_m"),
        ),
        bank=BankAndFill(
            bank_energy_kj=_number(bank, "bank_energy_kj"),
            peak_current_ma=_number(bank, "peak_current_ma"),
            fill_pressure_torr=_number(bank, "fill_pressure_torr"),
            deuterium_fill=_boolean(bank, "deuterium_fill"),
        ),
        registry=RegistryBinding(
