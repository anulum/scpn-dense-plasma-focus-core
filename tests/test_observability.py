...
# SCPN Dense Plasma Focus Core — diagnostic and clock semantics tests
...
from scpn_dense_plasma_focus_core.errors import DiagnosticPlanError
...
EVENT_BINDINGS = {
    ...
    "event_reference": "synthetic discharge-start marker",
    "provenance": "synthetic fixture",
...
REFERENCE_FRAMES = (
    ReferenceFrame(
        identifier="frm_electrode_axis",
        kind=FrameKind.MACHINE_CYLINDRICAL,
        description="coaxial-electrode cylindrical frame",
    ),
)
CLOCK_RELATIONS = (
    ClockRelation(
...
SIGNALS_CH_DISCHARGE_CURRENT_TRAIN = (
    SignalDeclaration(
        identifier="sig_discharge_cycle",
    ...
    SignalDeclaration(
        identifier="sig_discharge_duration",
        quantity="time",
        unit="s",
    ...
    SignalDeclaration(
        identifier="sig_discharge_onset",
        quantity="time",
        unit="s",
...
SIGNALS_CH_NECK_MODE_ARRAY = (
    SignalDeclaration(
        identifier="sig_mode_amplitude",
        quantity="magnetic_flux_density",
        unit="T",
        role=SignalRole.AMPLITUDE,
        description="synthetic neck-mode amplitude",
    ),
    SignalDeclaration(
...
def channel_event_train() -> DiagnosticChannelPlan:
    """Build the synthetic discharge current/voltage event channel."""
    return DiagnosticChannelPlan(
        identifier="ch_discharge_current_train",
...
def test_candidate_rejects_malformed_identifier() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="candidate_id"):
        CandidateProfile(
            candidate_id="Bad-Id",
            phenomenon="x",
            configurations=("dense_plasma_focus",),
...
def test_candidate_rejects_unsorted_configurations() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        CandidateProfile(
            candidate_id="a.b",
            phenomenon="x",
            configurations=("dense_plasma_focus", "dense_plasma_focus"),
            observability_class=ObservabilityClass.EVENT_RELATIVE,
        )
...
def _transformation(**overrides: Any) -> FrameTransformation:
    ...
    values: dict[str, Any] = {
        "source_identifier": "frm_electrode_axis",
...
def test_channel_rejects_unsorted_or_duplicate_signals() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        _derived(signals=tuple(reversed(SIGNALS_CH_NECK_MODE_ARRAY)))
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        _derived(signals=(*SIGNALS_CH_NECK_MODE_ARRAY, SIGNALS_CH_NECK_MODE_ARRAY[-1]))
...
def test_non_event_channel_rejects_timing_marker() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="only event-relative"):
        _derived(signals=(*SIGNALS_CH_NECK_MODE_ARRAY, marker))
...
def test_transformation_rejects_self_mapping() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="to itself"):
        _transformation(target_identifier="frm_electrode_axis")
...
def test_report_flags_cyclic_array_without_amplitude_signal() -> None:
    ...
    channel = _derived(
        signals=tuple(
            signal
            for signal in SIGNALS_CH_NECK_MODE_ARRAY
...
def test_channel_rejects_nyquist_violation() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="Nyquist"):
        _derived(sample_rate_hz=5.0e07)
...
def test_report_flags_mhd_band_outside_typical_range() -> None:
    ...
    findings = plan.consistency_report()
    assert len(findings) == 1
    assert "Lee" in findings[0].message
...
def test_report_flags_window_beyond_device_ceiling() -> None:
    ...
    channel = _derived(acquisition_duration_s=0.001)
    plan = synthetic_plan()
    plan = DiagnosticPlan(
        identifier=plan.identifier,
        binding=plan.binding,
