...
# SCPN Dense Plasma Focus Core — vendored transcendental kernel tests

"""Accuracy, exactness, identities and refusals of the vendored ``ln``/``exp``.

The module under test is the temporary copy of the shared kernel library's
transcendental kernel (canonical: SCPN-REACTOR-KERNELS, commit 799d44d3);
these tests are the library's own tests with the import adjusted, so the
copy carries the same evidence until the library is pinned. The accuracy
bounds are the evidence record quoted in ``VALIDATION.md``; the platform
``math`` module is the reference. Every value is synthetic.
"""
...
def test_numerics_error_is_a_device_configuration_error() -> None:
    """The refusal type sits under the device configuration error."""
    from scpn_dense_plasma_focus_core.errors import DeviceConfigurationError

    assert issubclass(NumericsError, DeviceConfigurationError)
    assert issubclass(NumericsError, ValueError)


def test_copy_names_its_canonical_origin() -> None:
    """The vendored module records the library commit it was taken from."""
    import scpn_dense_plasma_focus_core.physics._transcendental as module

    assert module.__doc__ is not None
    assert "799d44d3" in module.__doc__
    assert "SCPN-REACTOR-KERNELS" in module.__doc__
